...
use crate::{
    AacAudioConfig, AacAudioData, AudioCodec, AudioTag, AudioTagAacPacketType,
    AudioTagSampleSize, AudioTagSoundRate, GenericAudioData, RtmpMessageParseError,
...
impl AudioMessage {
    pub(super) fn from_raw(msg: RawMessage) -> Result<Self, RtmpMessageParseError> {
        ...
        let msg = match (tag.codec, tag.aac_packet_type) {
            (AudioCodec::Aac, Some(AudioTagAacPacketType::Data)) => {
                Self::AacData(AacAudioData {
