...
use crate::{
    BytesDecoder, BytesEncoder, DecoderError, RawFrameData, VulkanDecoderError,
...
pub(crate) const DECODE_EXTENSIONS: &[&CStr] =
    &[vk::KHR_VIDEO_DECODE_QUEUE_NAME, vk::KHR_VIDEO_DECODE_H264_NAME];

pub(crate) const ENCODE_EXTENSIONS: &[&CStr] =
    &[vk::KHR_VIDEO_ENCODE_QUEUE_NAME, vk::KHR_VIDEO_ENCODE_H264_NAME];
...
impl VulkanDevice {
    pub(crate) fn new(
        ...
        let h264_encode_queues =
            queue_indices.h264_encode.as_ref().map_or(Vec::new(), |queue_family_index| {
                (0..queue_family_index.queue_count)
                    .map(|idx| queue_from_device(device.clone(), queue_family_index, idx))
                    .collect::<Vec<_>>()
            });
        ...
        let queues = Queues {
            ...
            h264_decode: VideoQueues::new(h264_decode_queues.into_boxed_slice())
                .map(Arc::new),
            h264_encode: VideoQueues::new(h264_encode_queues.into_boxed_slice())
                .map(Arc::new),
            wgpu: wgpu_queue,
        };
    ...
    pub fn create_bytes_encoder(
        ...
        Ok(BytesEncoder { vulkan_encoder: encoder })
    }
