...
use libva::{
    Buffer, Context, Display, DrmPrimeSurfaceDescriptor, Picture, PictureEnd,
    PictureNew, PictureSync, Surface, SurfaceMemoryDescriptor, VaError,
};
...
pub(crate) fn va_surface_id<V: VideoFrame>(
    ...
    match handle {
        ...
        Some(handle) => RefCell::borrow(handle).surface().id(),
    }
}

impl<V> DecodedHandleTrait for DecodedHandle<V>
where
    V: VideoFrame,
    V::NativeHandle: StdBorrow<Surface<V::MemDescriptor>>,
{
    type Frame = V;

    fn video_frame(&self) -> Arc<Self::Frame> {
        RefCell::borrow(self).backing_frame.clone()
    }

    fn coded_resolution(&self) -> Resolution {
        RefCell::borrow(self).surface().size().into()
    }

    fn display_resolution(&self) -> Resolution {
        RefCell::borrow(self).display_resolution
    }

    fn timestamp(&self) -> u64 {
        RefCell::borrow(self).timestamp()
    }

    fn is_ready(&self) -> bool {
        RefCell::borrow(self).state.is_ready().unwrap_or(true)
    }

    fn sync(&self) -> anyhow::Result<()> {
        RefCell::borrow_mut(self).sync().context("while syncing picture")?;
...
enum PictureState<M: SurfaceMemoryDescriptor, H: StdBorrow<Surface<M>>> {
    Ready(Picture<PictureSync, H>),
    Pending(Picture<PictureEnd, H>),
    // Only set in sync when we take ownership of the VA picture.
    Invalid(PhantomData<M>),
}

impl<M, H> PictureState<M, H>
where
    M: SurfaceMemoryDescriptor,
    H: StdBorrow<Surface<M>>,
{
    /// Make sure that all pending operations on the picture have completed.
    fn sync(&mut self) -> Result<(), VaError> {
        let res;

        (*self, res) = match std::mem::replace(
            self,
            PictureState::Invalid(PhantomData),
        ) {
            ...
            PictureState::Invalid(_) => unreachable!(),
        };
    ...
    fn new_from_same_surface(&self, timestamp: u64) -> Picture<PictureNew, H> {
        match &self {
...
pub struct VaapiDecodedHandle<V: VideoFrame> {
    backing_frame: Arc<V>,
    state: PictureState<<V as VideoFrame>::MemDescriptor, <V as VideoFrame>::NativeHandle>,
    /// Actual resolution of the visible rectangle in the decoded buffer.
    display_resolution: Resolution,
}

impl<V> VaapiDecodedHandle<V>
where
    V: VideoFrame,
    V::NativeHandle: StdBorrow<Surface<V::MemDescriptor>>,
{
    ...
    /// Exports the decoded VA surface as DRM PRIME descriptors.
    pub fn export_prime(&self) -> Result<DrmPrimeSurfaceDescriptor, VaError> {
        self.surface().export_prime()
    }

    /// Returns the VA surface ID backing this decoded frame.
...
pub struct VaapiPicture<V: VideoFrame> {
    picture: Picture<PictureNew, V::NativeHandle>,
    backing_frame: Arc<V>,
}

impl<V> VaapiPicture<V>
where
    V: VideoFrame,
    V::NativeHandle: StdBorrow<Surface<V::MemDescriptor>>,
{
