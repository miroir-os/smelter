...
impl RawDataInput {
    pub fn new_input(
        ...
        let (video_sender, video_receiver) = match options.video {
            true => {
                let (sender, receiver) = spawn_video_repacking_thread(
                    ctx.clone(),
                    &input_ref,
                    buffer_duration,
                );
        ...
        let (audio_sender, audio_receiver) = match options.audio {
            true => {
                let (sender, receiver) = spawn_audio_repacking_thread(
                    ctx.clone(),
                    &input_ref,
                    buffer_duration,
                );
            ...
            for event in input_receiver.into_iter() {
                let event = match event {
                    PipelineEvent::Data(mut frame) => {
                        let start_pts = *start_pts
                            .get_or_insert_with(|| ctx.queue_sync_point.elapsed());
                        let first_frame_pts = *first_frame_pts.get_or_insert(frame.pts);
                        frame.pts =
                            frame.pts + start_pts + buffer_duration - first_frame_pts;
...
fn spawn_audio_repacking_thread(
    ...
    input_ref: &Ref<InputId>,
    buffer_duration: Duration,
) -> (Sender<PipelineEvent<InputAudioSamples>>, Receiver<PipelineEvent<InputAudioSamples>>)
{
    let (output_sender, output_receiver) = bounded(5);
                ...
                let event = match event {
                    PipelineEvent::Data(mut batch) => {
                        let start_pts = *start_pts
                            .get_or_insert_with(|| ctx.queue_sync_point.elapsed());
                        let first_frame_pts =
                            *first_frame_pts.get_or_insert(batch.start_pts);
                        batch.start_pts = batch.start_pts + start_pts + buffer_duration
                            - first_frame_pts;
