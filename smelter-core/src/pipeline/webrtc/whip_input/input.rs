...
/// ### On input register
///
/// Nothing happens, waiting for first connection.
///
/// ### On connection
/// - PTS of first frame should be `queue_sync_point.elapsed()` (at the time of
///   `queue_new_track`)
/// - Register track with `QueueTrackOffset::Pts(Duration::ZERO)`
/// - Jitter buffer: configured via `opts.jitter_buffer`
///
